...
@section('content')
    <div style="text-align: center">
        <h2 class="small-title">All products</h2>
    </div>
    ...
    <section class="scroll-section" id="hoverableRows">
        <div class="card mb-5">
            <div class="card-body">
                <div>
                    <a class="btn btn-primary col-md-2" href={{route("stor")}}>
                        New store
                    </a>
                </div>
                <br />
                ...
                <table class="table table-bordered table-hover table-sm ListTable">
                    <thead class="datagridheader">
                        <tr>
                            <th scope="col">stor.No</th>
                            <th scope="col">store Name</th>
                            <th scope="col">store Discreption</th>
                    ...
                    <tbody>
                        ...
                        @foreach ($store as $rowNo => $store)
                        
                            <tr class="tr">
                                
                                <th scope="row">{{ $rowNo + 1 }}</th>
                                <td>{{$store->name}}</td>
                                <td>{{$store->discreptin}}</td>
                                
                                <td>
                                    <div class="how-itemcart1">
                                    <img src="{{ $store->image_path }}"  alt="IMG"/>
                                </td>
                                <td>
                                    <a class="btn btn-sm btn-icon btn-icon-start btn-outline-primary ms-1"
                                        href="{{ route('EditStore', ['id' => $store->id]) }}" title="Edit">
                                    ...
                                    <a class="btn btn-sm btn-icon btn-icon-start btn-outline-danger ms-1"
                                        href="{{ route('DeleteStore', ['id' => $store->id]) }}" title="Delete">
                                        <i data-acorn-size="15">
                                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16"
                                    ...
                                    <a class="btn btn-sm btn-icon btn-icon-start btn-outline-primary ms-1"
                                        href="{{ route('DetailStore', ['id' => $store->id]) }}" title="Details">
                                        <i data-acorn-size="15">
